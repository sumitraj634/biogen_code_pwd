...
export class OrderReleaseService implements WebWorkerInputService {
  public currentInstanceOf = 'OrderReleaseService';
  public dragdropList = [];
  ...
  public async releaseValueChange(element) {
    ...
    this.dragdropList = [...this.releasedragdropList, ...this.linedragdropList];
    return await this.transmissionService.updateRelease(element);
  }
  ...
  async onUpload(InputFileEvent) {
    this.itemIdIndex = this.dragdropListTc.findIndex(
      (d) => d.path[0].includes('ReleaseGid') && d.path[0].includes('Xid')
    );
    this.itemIdDomain = this.dragdropListTc.findIndex(
      (d) => d.path[0].includes('ReleaseGid') && d.path[0].includes('DomainName')
    );
    const webWorkerProps: WebWorkerInputProps = {
      dragdropList: this.dragdropListTc,
      containLines: true,
      parentGidIdentifierPathString: 'ReleaseGid',
      lineIdentifierPathString: 'ReleaseLine',
    };
  ...
  public async sendDataToGtm() {
    await this.transmissionService.sendDataToGtm(this, 'ReleaseGid');
  }
  getViewUrl = (viewURL, i, statusInitial) => {
    ...
    return viewURL
      .replace('GTM_OTM_OBJECT', 'OrderRelease')
      .replace('GTM_OTM_MANAGER_LAYOUT_GID', 'ORDER_RELEASE_WO_STOP_VIEW')
      .replace('GTM_OTM_FINDER_SET', 'ORDER_RELEASE_WO_STOP')
      .replace(
        'GTM_OTM_OBJECT_GID',
