...
const RELEASE_MANDATORY_FIELD = [
  'Release ID DomainName',
  'Release ID',
  'Release TransactionCode',
  'Line ID DomainName',
  'Line ID',
  'Line TransactionCode',
  ...Object.values(NAME.release),
];
...
export const mapHeader = (element, type, gtmVersion, xmlNs, domain) => {
  return {
    name: element.name,
    required: true,
    disabled: getMandatoryElement(element.name, RELEASE_MANDATORY_FIELD),
    display: getMandatoryElement(element.name, RELEASE_MANDATORY_FIELD),
    isMandatory: getMandatoryElement(element.name, RELEASE_MANDATORY_FIELD),
    type: type,
